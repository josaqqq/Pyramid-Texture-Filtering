...
[[Paper]](https://arxiv.org/pdf/2305.06525)
[[Project Page]](https://rewindl.github.io/pyramid_texture_filtering/)
...
しかし，ナイーブにアップサンプリングを施すだけでは，元画像に含まれたシャープなストラクチャを再現することができない。
そこで，筆者らは Gaussian Pyramid と Laplacian Pyramid を元にして Joint Bilateral Filter (JBF) によるアップサンプリングを施す手法を提案した。
これにより，アップサンプリングの中で元画像のシャープなストラクチャが消失する問題を解決し，高精度なテクスチャフィルタリングを実現した。
...
Laplacian Pyramid 構築の際は，まず Gaussian Pyramid を構築し，各レベルの Gaussian Pyramid 画像を高解像度のレベルにアップサンプリングする。
アップサンプリングされた画像と，対応する高解像度レベルの Gaussian Pyramid 画像との差分を取る。
この差分画像が，Laplacian Pyramid の各レベルにおける画像となる。
Laplacian Pyramid では，Gaussian Pyramid 構築の際に行われるダウンサンプリングで失われる情報を保持している。
...
アップサンプリングの際は，Gaussian Pyramid 中の対応する画像をガイド画像，アップサンプリング元の画像を対象画像として JBF を施す。
...
しかし，Gaussian Pyramid と Laplacian Pyramid を基礎にして，ダウンサンプリングとアップサンプリングを行う本手法では，そのようなブラーやアーティファクトを出現させることなくテクスチャフィルタリングを施すことができる。

また，本手法では，予めモデルを学習させたり，元画像でストラクチャの境界を指定する必要はなく，ダウンサンプリングとアップサンプリングによる簡単な画像処理のみで高精度なテクスチャフィルタリングを実現できる。
Gaussian Pyramid と Laplacian Pyramid を用いたテクスチャフィルタリングでは，JBFのハイパーパラメータのみを指定すればよく，従来手法よりも扱いやすい。
...
続いて，JBF を用いたアップサンプリングを実装した。
具体的には，Pyramid 構築において実装した Bilinear Interpolation と新たに実装した JBF によってアップサンプリングを施した。

Gaussian Pyramid によるダウンサンプリングと，Gaussian Pyramid，Laplacian Pyramid，JBF によるアップサンプリングにより，提案手法全体のパイプラインを実装した。

## 実験結果
以下は，実装したパイプラインによるテクスチャフィルタリングの結果である。
提案手法のパイプラインでは，Joint Bilateral Filtering のパラメータが存在するが，それらは $\sigma_s = 9.0, \sigma_r = 0.09$ に統一して実験を行った。
実験では，自身が撮影した実世界の画像を $1024 \times 1024$ にリサイズして用いた。
...
### プログラム実行
以下のコマンドを実行してプログラムを実行する。
出力ファイルは，`./build/output`に出力される。
```
cd ./build
mkdir ./output
mkdir ./debug
./PyramidTextureFiltering ../input/<image-file-name>.png
```

### 入力画像作成
独自に用意した画像で実験する場合は，$1024 \times 1024$等の十分に大きな2のべき乗のサイズにリサイズし，`../input/<image-file-name>.png`として配置する。

### 実験結果の再現
ルートディレクトリにある`./bash.sh`を実行することで，実験結果を再現することができる。
```
./bash.sh
```
